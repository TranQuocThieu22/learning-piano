...
**All 25 exercise lessons (Chương 1-7) follow this layout — copy any of them.** Chương 1-2 were the pilot; Chương 3-7 were rewritten the same day after the owner approved it.
...
- **A new concept gets a tiny picture of its own**, placed right under the opening paragraph: a 1-bar `abc` staff that shows only the new mark (the notes on the staff, a slur, a staccato dot, `!p!`…`!f!`), or a `keys` diagram for a hand shape or black key (`F4 F#4`, `C3 Eb3 G3`). Chương 3-7 each have one — look at them. These extra blocks must still pass `check:lessons` (bar lengths, note range of the chapter).
- **`{{sheet: …}}` only finds headings of the form `## Bài tập <code>`**, so a recital's `## 🎵 Tiết mục …` cannot be embedded. Warm up from a recital with a link to the lesson instead.
- **Hard limits, enforced by `pnpm check:lessons`** for **every** exercise lesson (a lesson without the `## Tập thấy khó? Đọc ở đây` heading is itself an error): opening paragraph ≤ 450 characters, prose in one practice section ≤ 350, one paragraph in the read-more section ≤ 300, and every `## Bài tập` section must contain a staff, a diagram, an image or a sheet embed. The limits sit just above the longest Chương 1-2 lesson at the time they were set. Over the limit → cut words or turn them into a picture; **never raise the number to make a lesson pass**.
...
**Theory chapters (`docs/02-chapters/chuong-XX.md`) are not steps.** They are not on the learning path, not tickable, and open with a quote block *"> **Nên tập thử trước khi đọc trang này.** …"* plus a link to the chapter's first exercise, and end with *"👉 Về tập tiếp: …"*. Never write an exercise that only makes sense after reading the theory chapter, and never tell the learner to read it first.

# "Xong bài khi" — every exercise lesson MUST have it
- Put a `## Xong bài khi` section **right after the last exercise**, before the `---` that opens the read-more section. It holds **2 to 4 bullet lines** (`- ...`), each a concrete, self-checkable outcome: *"Đánh trọn 2E từ nốt đầu tới nốt cuối — chậm cũng được, không dừng lại để sửa."*
